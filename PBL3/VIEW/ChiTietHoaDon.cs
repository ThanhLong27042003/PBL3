...
namespace PBL3.VIEW
{
    ...
    {
        public ChiTietHoaDon(string idhoadon)
        {
            InitializeComponent();
            setValue(idhoadon);
        }

        public void setValue(string IDHOADON)
        {
            ChiTietHoaDonBLL bll = new ChiTietHoaDonBLL();

            textBox1.Text = bll.LayInfoDong(IDHOADON).Mã_hóa_đơn;
            textBox2.Text = bll.LayInfoDong(IDHOADON).Mã_tài_khoản;
